...
namelist = ["IID, C = 0.1, K = 100, E = 1",#0-#3. G1-G4
            ...
            "IID, C = 0.5, K = 100, E = 1",
            "IID, C = 1, K = 100, E = 1",
            "non-IID, C = 0.1, K = 100, E = 1", #4-#7, G5-G8
            "non-IID, C = 0.2, K = 100, E = 1", 
            "non-IID, C = 0.5, K = 100, E = 1", 
            "non-IID, C = 1, K = 100, E = 1", 
            "non-IID, C = 0.1, K = 100, E = 10", #8-#11, G9-G12
            ...
            "IID, C = 0.1, K = 100, E = 10",#16-#19, G17-G20
            "IID, C = 0.2, K = 100, E = 10",
            "IID, C = 0.5, K = 100, E = 10",
            "IID, C = 1, K = 100, E = 10", 
            "IID, C = 0.1, K = 100, E = 20",#20-#23, G21-G24
...
def PlotAcc(acclists, labels, iidno, noniidno, epochs = [1] * len(namelist)):
    ...
    ax.axhline(y=0.97, color='gray', linestyle='--')
    ax.text(0, 0.97, 'Accuracy Threshold = 0.97', ha='left')
...
def PlotECEVSOAcc(values, indices):
    ecelist = [values[3 + 7 * (i - 1)] for i in indices]
    oacclist = [values[7 * (i - 1)] for i in indices]
    labels = ["G" + str(i) + ", " + namelist[i - 1] for i in indices]
    innerPlotECEVSOAcc(ecelist, oacclist, labels)

def innerPlotECEVSOAcc(ecelist, oacclist, labels):
    fig = plt.figure()
    ...
    for i in range(len(ecelist)):
        ax.plot(range(1, len(ecelist[i]) + 1), ecelist[i], 
                color=cmap(i / (len(ecelist) - 1)), 
    ...
    for i in range(len(oacclist)):
        ax.plot(range(1, len(oacclist[i]) + 1), oacclist[i], 
                color=cmap(i / (len(oacclist) - 1)), 
