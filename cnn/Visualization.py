...
namelist = ["IID, C = 0.1, K = 100, E = 1",#G1-G3
            "IID, C = 0.5, K = 100, E = 1",
            "IID, C = 1, K = 100, E = 1",

            "IID, C = 0.1, K = 100, E = 10",#G4-G6
            "IID, C = 0.5, K = 100, E = 10",
            "IID, C = 1, K = 100, E = 10", 

            "IID, C = 0.1, K = 100, E = 20",#G7-G9
            "IID, C = 0.5, K = 100, E = 20",
            "IID, C = 1, K = 100, E = 20",

            "non-IID, C = 0.1, K = 100, E = 1", #G10-G12
            "non-IID, C = 0.5, K = 100, E = 1", 
            "non-IID, C = 1, K = 100, E = 1", 

            "non-IID, C = 0.1, K = 100, E = 10", #G13-G15
            "non-IID, C = 0.5, K = 100, E = 10", 
            "non-IID, C = 1, K = 100, E = 10", 

            "non-IID, C = 0.1, K = 100, E = 20", #G16-G18
...
def PlotDataDistribution(iid_train_loader, noniid_train_loader):
    ...
    iid_labels = torch.stack(iid_labels).t()
    ...
    noniid_labels = torch.stack(noniid_labels).t()
    noniid_normalized = noniid_labels / 600
    
    plt.figure(figsize=(16, 6))
    
    plt.subplot(2, 1, 1)
    sns.heatmap(iid_normalized.numpy(), cmap='Reds', fmt=".2f", cbar=True, annot=False)
    plt.xticks(np.arange(0, 100, step=10), [i * 10 for i in range(10)])
    plt.ylabel('Classes', fontsize=18)
    plt.xlabel('Clients No.', fontsize=18)
    plt.title('IID Scenarios', fontsize=24)
    
    plt.subplot(2, 1, 2)
    sns.heatmap(noniid_normalized.numpy(), cmap='Reds', fmt=".2f", cbar=True, annot=False)
    plt.xticks(np.arange(0, 100, step=10), [i * 10 for i in range(10)])
    plt.ylabel('Classes', fontsize=18)
    plt.xlabel('Clients No.', fontsize=18)
    plt.title('Non-IID Scenarios', fontsize=24)
...
def PlotUncertaintyVSAcc(binEntro_of_rounds, binAcc_of_rounds, label, counts, binsize):
    ...
    for i in range(len(binEntro_of_rounds)):
        ...
        for j in range(len(binEntro_of_rounds[i])):
            if counts[i][j] >= binsize:
                ax.scatter(binAcc_of_rounds[i][j], binEntro_of_rounds[i][j], color=color, s=3)
    ...
    interval = max(len(binEntro_of_rounds) // 10, 1)
    cbar.ax.yaxis.set_major_locator(MultipleLocator(interval))

    plt.show()

def PlotPPVSAcc(binPPo_of_rounds, binAcc_of_rounds, label, counts, binsize):
     ...
     for i in range(len(binPPo_of_rounds)):
         ...
         for j in range(len(binPPo_of_rounds[i])):
             if counts[i][j] >= binsize:
                 ax.scatter(binAcc_of_rounds[i][j], binPPo_of_rounds[i][j], color=color,  s = 3)
     ...
     interval = max(len(binPPo_of_rounds) // 10, 1)
     cbar.ax.yaxis.set_major_locator(MultipleLocator(interval))
